...
ただし利便性を考慮して標準の `DefaultHTTPConnector` を用意しているので、シビアな要件がなければ独自実装をする必要はありません。


## 通信に対するアプリケーションの共通的なふるまい

ほとんどのアプリケーションには、通信エラー時にポップアップを表示するなど、複数の通信で共通して行うふるまいがあります。  
本ライブラリではそのような共通のふるまいを実装するためのプロトコルを3種類用意しています。
これらを使使って複数のAPIに共通のふるまいを実装することができます。  
