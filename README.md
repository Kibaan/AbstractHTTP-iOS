...
HTTP通信処理もタイムアウト時間、キャッシュの仕方、クッキーの扱い、SSL証明書の扱いなどいくつか典型的な固有要素がありますが、本ライブラリはそれらにあまり手出ししません。

HTTP通信処理はシンプルなプロトコル`HTTPConnector`にしていますので、各アプリケーションの要件に従って自由に実装してください。  
...
ただし利便性を考慮して標準実装の `DefaultHTTPConnector` を用意しているので、シビアな要件がなければ独自実装をする必要はありません。


## 通信に対するアプリケーションの共通的なふるまい

ほとんどのアプリケーションには、通信エラー時にポップアップを表示するなど、複数の通信で共通して行うふるまいがあります。  
本ライブラリではそのような共通のふるまいを実装するためのプロトコルを3種類用意しており、これらを使使って複数のAPIに共通のふるまいを実装することができます。  
...
`ConnectionResponseListener.onReceived` などのコールバック関数で通信のリトライをする場合、一連のコールバック関数が呼び出される順番は以下の2パターンがあります。

#### onEndまで呼び出された後に再通信する

アラートを表示してリトライボタンを押した後に再通信する場合など、コールバック関数内でただちに再通信を行わずスレッドを明け渡してから再通信する場合、コールバック関数は一度最後の `ConnectionListener.onEnd` まで実行され、その後再通信によりもう一度最初からコールバック関数が呼び出されます。

このケースでは`onStart` から `onEnd` までの流れが2セット実行されます。

#### コールバックの途中で再通信する

`ConnectionResponseListener.onReceived` などのコールバック関数内でただちに再通信を行った場合、1度目の通信によるコールバック呼び出しは中断され、再通信によるコールバック呼び出しが行われますが、この際`onStart`はスキップされ呼ばれません。
 
このケースでは2回通信を行いますが `onStart`、`onEnd` はそれぞれ1度しか実行されず、リスナーに対して1回しか通信が行われていないようにふるまいます。
